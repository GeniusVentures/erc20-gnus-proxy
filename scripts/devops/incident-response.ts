...
class IncidentResponseSystem {
  ...
  async executeAction(incident: Incident, action: string): Promise<void> {
    ...
    if (actionMap[action]) {
      try {
        ...
        this.log(
          `Error executing action ${action}: ${(error as Error).message}`,
          "error",
        );
      }
    }
  ...
  calculatePriority(incidentData: IncidentData): number {
    ...
    const severityWeight =
      severityWeights[incidentData.severity || "medium"] || 2;
    const categoryWeight =
      categoryWeights[incidentData.category || "general"] || 1;
  ...
  async getActiveIncidents(): Promise<Incident[]> {
    ...
    try {
      ...
      for (const file of files) {
        if (file.startsWith("incident-") && file.endsWith(".json")) {
          try {
            ...
            if (
              this.validateIncidentId(incident.id) &&
      ...
      this.log(
        `Error loading active incidents: ${(error as Error).message}`,
        "error",
      );
    }
...
async function main(): Promise<void> {
  ...
  switch (command) {
    case "create":
      ...
      const severity =
        (args[3] as "low" | "medium" | "high" | "critical") || "medium";
    ...
    case "update":
      ...
      if (
        !updateStatus ||
        !["active", "investigating", "resolved", "closed"].includes(
          updateStatus,
        )
      ) {
        console.error(
