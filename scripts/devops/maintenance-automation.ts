...
class MaintenanceAutomation {
  ...
  async runScheduledTasks(
    frequency: keyof MaintenanceSchedule,
  ): Promise<TaskResult[]> {
  ...
  switch (command) {
    ...
    case "run":
      ...
      if (!frequency || !["daily", "weekly", "monthly"].includes(frequency)) {
        console.error(
          "Error: Valid frequency required (daily, weekly, monthly)",
        );
    ...
    case "list":
      ...
      Object.entries(listSchedule).forEach(
        ([freq, tasks]: [string, MaintenanceTask[]]) => {
          console.log(`\n${freq.toUpperCase()}:`);
          if (tasks.length === 0) {
    ...
    default:
      console.log("Usage:");
      console.log(
        "  npx ts-node scripts/devops/maintenance-automation.ts setup",
      );
      console.log(
        "  npx ts-node scripts/devops/maintenance-automation.ts run <frequency>",
      );
      console.log(
        "  npx ts-node scripts/devops/maintenance-automation.ts list",
      );
      console.log("");
