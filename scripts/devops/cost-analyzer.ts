...
class CostAnalyzer {
  ...
  constructor() {
    this.costData = [];
    this.costFile = path.join(
      process.cwd(),
      ".github/costs/workflow-costs.jsonl",
    );
  ...
  loadCostData(): void {
    if (!fs.existsSync(this.costFile)) {
      console.log(
        "📄 No cost data file found. Creating initial cost tracking file...",
      );
  ...
  analyzeCosts(timeframe: string = "all"): CostAnalysis {
    ...
    if (timeframe !== "all") {
      ...
      filteredData = this.costData.filter(
    ...
    const workflowCosts: WorkflowCost[] = Object.entries(byWorkflow)
      .map(([workflow, runs]) => ({
        ...
        avgCost:
          runs.reduce((sum, run) => sum + run.estimated_cost, 0) / runs.length,
        avgRuntime:
          runs.reduce((sum, run) => sum + run.runtime_minutes, 0) / runs.length,
      }))
  ...
  generateReport(timeframe: string = "month"): {
    ...
    console.log(
      `📊 Generating cost analysis report for timeframe: ${timeframe}`,
    );
...
`;
    ...
    const reportPath = path.join(
      reportsDir,
      `cost-analysis-report-${timeframe}.md`,
    );
  ...
  async run(): Promise<{ report: string; analysis: CostAnalysis }[]> {
    try {
      ...
      const reports = ["week", "month"].map((timeframe) =>
        this.generateReport(timeframe),
      );

      console.log("\n🎉 Cost analysis completed successfully!");
      console.log(
        "📊 Check cost-analysis-report-*.md files for detailed insights",
      );
