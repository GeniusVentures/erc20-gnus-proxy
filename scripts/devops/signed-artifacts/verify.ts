...
class ArtifactVerifier {
  ...
  async checkIntegrity(): Promise<void> {
    ...
    for (const [artifactName, artifactInfo] of Object.entries(
    ...
    if (integrityIssues > 0) {
      console.error(
        `❌ Integrity check failed: ${integrityIssues} issues found`,
      );
      process.exit(1);
    }
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log("Usage:");
      console.log(
        "  npx ts-node verify.ts verify     # Verify cryptographic signature",
      );
      console.log(
        "  npx ts-node verify.ts integrity  # Check artifact integrity",
      );
      console.log("");
      ...
      console.log(
        "  integrity - Check that signed artifacts haven't been modified",
      );
      process.exit(1);
  }
}
