...
class SecurityHealthChecks {
    ...
    try {
      ...
      try {
        ...
        const vulnerabilities = (
          auditOutput.match(/severity: (critical|high)/g) || []
        ).length;
   ...
   */
  private async checkContractAudit(
    options: HealthCheckOptions = {},
  ): Promise<CheckResult> {
      ...
      for (const file of networkFiles) {
        if (file.endsWith(".json")) {
          const config = JSON.parse(
            fs.readFileSync(path.join(networksPath, file), "utf8"),
          );
