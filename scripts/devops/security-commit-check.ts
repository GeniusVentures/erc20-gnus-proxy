...
function validateCommitMessageFile(filePath: string): string {
  ...
  if (resolvedPath.startsWith(cwd) || resolvedPath.startsWith(gitDir)) {
    ...
    const isAllowedTemp = allowedPrefixes.some((prefix) =>
      resolvedPath.startsWith(prefix),
    );

    if (!isAllowedTemp) {
      console.error(
        "❌ Invalid commit message file path: not in allowed location",
      );
      process.exit(1);
    }
  }
  ...
  try {
    ...
    console.error(
      "❌ Cannot access commit message file:",
...
try {
  const commitMsg: string = fs
    .readFileSync(validatedCommitMsgFile, "utf8")
    .trim();
  ...
  if (isSecurityCommit) {
    ...
    const failedRules: SecurityValidationRule[] =
      securityValidationRules.filter(
  ...
  console.error(
    "❌ Error validating security commit:",
    (error as Error).message,
  );
  process.exit(1);
}
