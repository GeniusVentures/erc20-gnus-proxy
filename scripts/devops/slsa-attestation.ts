...
class SLSAAttestation {
   ...
   * Create DSSE envelope
   */
  private async createDSSEEnvelope(
  ...
  async verifyAttestation(attestationFile: string): Promise<boolean> {
    ...
    if (!fs.existsSync(validatedAttestationFile)) {
      throw new Error(
        `Attestation file not found: ${validatedAttestationFile}`,
      );
    }
  ...
  private verifyEnvelopeStructure(envelope: DSSEEnvelope): void {
    const requiredFields: (keyof DSSEEnvelope)[] = [
      "payload",
      "payloadType",
      "signatures",
    ];
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        "  node scripts/devops/slsa-attestation.ts verify [attestation-file]",
      );
