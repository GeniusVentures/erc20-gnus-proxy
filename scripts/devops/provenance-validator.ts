...
class ProvenanceValidator {
  ...
  private async validateCriticalDependencies(): Promise<void> {
    ...
    for (const criticalPkg of this.criticalPackages) {
      if (allDeps[criticalPkg]) {
        try {
          await this.validatePackageProvenance(
            criticalPkg,
            allDeps[criticalPkg],
          );
        } catch (error) {
  ...
  private async validatePackageProvenance(
    ...
    try {
      ...
      if (fs.existsSync(pkgJsonPath)) {
        const pkgData: PackageData = JSON.parse(
   ...
   * Validate package metadata
   */
  private validatePackageMetadata(
    packageName: string,
    pkgData: PackageData,
  ): void {
  ...
  private async checkForMaliciousPackages(): Promise<void> {
    ...
    try {
      ...
      this.log(
        `Malicious package check failed: ${(error as Error).message}`,
        "warn",
      );
    }
  }
    ...
    try {
      ...
      if (deps.hardhat && !deps["@nomicfoundation/hardhat-toolbox"]) {
        this.results.warnings.push({
          ...
          package: "hardhat",
          issue:
            "Missing recommended peer dependency: @nomicfoundation/hardhat-toolbox",
          severity: "low",
        });
      }
      ...
      this.log(
        `Peer dependency validation failed: ${(error as Error).message}`,
        "warn",
      );
    }
  }
  ...
  private async generateProvenanceReport(): Promise<void> {
    const report: ProvenanceReport = {
      ...
      summary: {
        ...
        criticalIssues: this.results.failed.filter(
          (f) => f.severity === "critical",
        ).length,
        highIssues: this.results.failed.filter((f) => f.severity === "high")
          .length,
      },
  ...
  private generateRecommendations(): Recommendation[] {
    ...
    const hasGitDeps = this.results.warnings.some(
      (w) => w.type === "dependency-source",
    );
    if (hasGitDeps) {
      recommendations.push({
        priority: "low",
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const hasCriticalFailures = results.failed.some(
      (f) => f.severity === "critical",
    );
