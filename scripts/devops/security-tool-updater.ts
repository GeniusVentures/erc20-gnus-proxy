...
class SecurityToolUpdater {
  ...
  private async getPipLatestVersion(
    packageName: string,
  ): Promise<string | null> {
    try {
  ...
  private async getNpmLatestVersion(
    packageName: string,
  ): Promise<string | null> {
    try {
  ...
  private async getGoLatestVersion(
    packageName: string,
  ): Promise<string | null> {
  ...
  private async getBinaryLatestVersion(
    toolName: string,
  ): Promise<string | null> {
  ...
  private async updateOsvScannerBinary(
    ...
    try {
      ...
      throw new Error(
        `Failed to update osv-scanner binary: ${(error as Error).message}`,
      );
    }
  }
      ...
      if (update.status === "pending") {
        ...
        try {
          ...
          if (newVersion === update.latest) {
            ...
            successCount++;
            console.log(
              `✅ ${update.tool} updated successfully to ${newVersion}`,
            );
          } else {
  ...
  private generateReport(): UpdateReport {
    const report: UpdateReport = {
      ...
      summary: {
        ...
        skipped: this.updateLog.filter(
          (u) => u.status === "pending" && !u.error,
        ).length,
      },
    };
    ...
    const reportPath = path.join(
      process.cwd(),
      "security-tool-update-report.json",
    );
