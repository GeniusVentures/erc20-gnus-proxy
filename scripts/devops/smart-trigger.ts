...
class SmartTrigger {
  ...
  private analyzeContract(filePath: string): void {
    try {
      ...
      contractAnalysis.functions.forEach((func) =>
        this.changes.functions.add(func),
      );
    } catch (error) {
      console.warn(
        `Could not analyze contract ${filePath}:`,
  ...
  private assessComplexity(content: string): ComplexityScore {
    ...
    const stateVars = (
      content.match(
        /(uint|int|address|bool|string|bytes|mapping)\s+\w+\s*[;=]/g,
      ) || []
    ).length;
  ...
  printSummary(): void {
    ...
    Object.entries(report.triggers).forEach(([trigger, activated]) => {
      ...
      console.log(
        `${status} ${trigger.replace(/([A-Z])/g, " $1").toLowerCase()}`,
      );
    });
