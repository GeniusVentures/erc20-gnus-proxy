...
/**
 * Deploys the Diamond contract, which serves as the main entry point of the protocol.
 * This function deploys the DiamondCutFacet and ProxyDiamond contracts if they haven't been deployed yet,
...
export async function deployDiamond(networkDeployInfo: INetworkDeployInfo) {
  ...
  if (!networkDeployInfo.DiamondAddress) {
    // deploy Diamond
    const proxyDiamond = await ethers.getContractFactory(
      'contracts/ProxyDiamond.sol:ProxyDiamond',
    );
    const contractOwnerAddress = await contractOwner.getAddress();
    diamond = await proxyDiamond.deploy(contractOwnerAddress, dc.DiamondCutFacet.address);
    await diamond.deployed();
  } else {
    // If ProxyDiamond is already deployed, attach to it using the stored address in deployment info.
...
export async function afterDeployCallbacks(
  ...
  // Reference the ProxyDiamond contract instance for interaction
  ...
  for (const name of facetsPriority) {
    ...
    if (facetDeployInfo.upgradeInit && (facetDeployInfo.fromVersions?.includes(previousVersion || -1))) {
      ...
      initFunction = facetDeployInfo.deployInit as keyof ProxyDiamond; // Use the deploy initialization function
    }
    ...
    if (initFunction) {
      const funcSelector = getSighash(`function ${initFunction.toString()}`); // Get the function selector
      if (!funcSelector) {
