...
export class RPCDiamondDeployer {
   ...
   */
  public static getDiamondConfigFromHardhat(
    diamondName: string,
  ): DiamondPathsConfig {
    try {
   ...
   */
  public static getNetworkConfigFromHardhat(
    networkName: string,
  ...
  public static async getInstance(
    ...
    if (!this.instances.has(key)) {
      ...
      const hardhatDiamonds: DiamondPathsConfig = (
        hre as any
      ...
      config.diamondAbiPath =
        config.diamondAbiPath ||
        (hardhatDiamonds as any)?.diamondAbiPath ||
        "diamond-abi";
      ...
      if (config.verbose) {
        console.log(
          chalk.green(
            `✅ Created new RPCDiamondDeployer instance with key: ${key}`,
          ),
        );
      }
      ...
      console.log(
        chalk.blue(
          `♻️  Using existing RPCDiamondDeployer instance with key: ${key}`,
        ),
      );
    }
  ...
  public static createConfigFromEnv(
    ...
    for (const envVar of requiredEnvVars) {
      if (
        ...
        !overrides?.[
          envVar
            .toLowerCase()
            .replace("_", "") as keyof RPCDiamondDeployerConfig
        ]
      ) {
    ...
    const config: RPCDiamondDeployerConfig = {
      ...
      writeDeployedDiamondData:
        process.env.WRITE_DEPLOYED_DIAMOND_DATA !== "false", // Default to true
      ...overrides,
    };
  ...
  private static validateConfig(config: RPCDiamondDeployerConfig): void {
    ...
    if (
      config.maxRetries &&
      (config.maxRetries < 1 || config.maxRetries > 10)
    ) {
    ...
    if (
      config.retryDelayMs &&
      (config.retryDelayMs < 100 || config.retryDelayMs > 30000)
    ) {
  ...
  public async deployDiamond(): Promise<Diamond> {
    ...
    if (this.deployInProgress) {
      if (this.verbose) {
        console.log(
          chalk.blue(
            `⏳ Deployment already in progress for ${this.networkName}`,
          ),
        );
    ...
    try {
      if (this.verbose) {
        ...
        console.log(
          chalk.blue(
            `🌐 Network: ${this.networkName} (Chain ID: ${this.chainId})`,
          ),
        );
  ...
  public async validateConfiguration(): Promise<{
    ...
    try {
      ...
      if (
        ...
        !existsSync(this.config.configFilePath)
      ) {
        errors.push(
          `Configuration file not found: ${this.config.configFilePath}`,
        );
      }
    } catch (error) {
  ...
  private async validateConnection(): Promise<void> {
    try {
      ...
      if (this.verbose) {
        ...
        console.log(
          chalk.gray(`   Balance: ${Number(networkInfo.balance) / 1e18} ETH`),
        );
        console.log(
          chalk.gray(
            `   Gas Price: ${Number(networkInfo.gasPrice) / 1e9} gwei`,
          ),
        );
      ...
      throw new Error(
        `Network connection validation failed: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async printDeploymentSummary(): Promise<void> {
    ...
    console.log(
      chalk.blue(
        `📍 Diamond Address: ${deployedData?.DiamondAddress || "N/A"}`,
      ),
    );
    ...
    if (deployedData?.protocolVersion) {
      console.log(
        chalk.blue(`📋 Protocol Version: ${deployedData.protocolVersion}`),
      );
    }
