...
async function showConfigDetails(
  ...
  console.log(
    `⛽ Gas Multiplier: ${chalk.white(config.gasLimitMultiplier || "1.2")}`,
  );
  console.log(`🔄 Max Retries: ${chalk.white(config.maxRetries || "3")}`);
  console.log(
    `⏱️  Retry Delay: ${chalk.white(config.retryDelayMs || "2000")}ms`,
  );
...
async function showFacetDetails(diamond: Diamond): Promise<void> {
  ...
  for (const [facetName, facetData] of Object.entries(facets) as [
    ...
    if (facetData.funcSelectors && facetData.funcSelectors.length > 0) {
      console.log(
        `   🎯 Selectors: ${chalk.white(facetData.funcSelectors.length)}`,
      );
    }
...
async function performOnChainValidation(
  ...
  try {
    ...
    console.log(
      `🌐 Network: ${chalk.white(network.name)} (${network.chainId})`,
    );
    ...
    if (code.length > 2) {
      ...
      try {
        ...
        console.log(
          `🔧 On-Chain Facets: ${chalk.white(facetAddresses.length)}`,
        );
      } catch {
        console.log(
          chalk.yellow("⚠️  Could not retrieve on-chain facet count"),
        );
      }
    }
  } catch (error) {
    console.error(
      chalk.red(`❌ On-chain validation failed: ${(error as Error).message}`),
    );
  }
}
...
async function checkStatus(options: StatusOptions): Promise<void> {
  ...
  console.log(
    chalk.blue(`📊 Checking status of diamond "${config.diamondName}"...`),
  );
  ...
  console.log(chalk.blue("===================="));
  console.log(
    `💎 Diamond Address: ${chalk.white(diamondAddress || "Not deployed")}`,
  );
