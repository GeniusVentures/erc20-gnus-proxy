...
async function collectMissingConfig(
  ...
  if (!options.diamondName) {
    options.diamondName = await prompt("Diamond name", "ProxyDiamond");
  }
...
async function preDeploymentConfirmation(
  ...
  console.log(
    chalk.blue(`💰 Balance: ${Number(networkInfo.balance) / 1e18} ETH`),
  );
  console.log(
    chalk.blue(`⛽ Gas Price: ${Number(networkInfo.gasPrice) / 1e9} gwei`),
  );

  if (deployConfig.protocolVersion) {
    console.log(
      chalk.blue(`📋 Protocol Version: ${deployConfig.protocolVersion}`),
    );
  }
...
async function stepByStepDeployment(
  ...
  try {
    ...
    if (deployedData.DiamondAddress) {
      console.log(
        chalk.blue(`📍 Diamond Address: ${deployedData.DiamondAddress}`),
      );
    }
  } catch (error) {
    console.error(
      chalk.red(`❌ Diamond deployment failed: ${(error as Error).message}`),
    );
    throw error;
  }
...
async function manualDeploy(options: ManualDeploymentOptions): Promise<void> {
  try {
    ...
    console.log(
      chalk.green(`📍 Diamond Address: ${deployedData.DiamondAddress}`),
    );

    if (deployedData.protocolVersion) {
      console.log(
        chalk.green(`📋 Protocol Version: ${deployedData.protocolVersion}`),
      );
    }
  } catch (error) {
