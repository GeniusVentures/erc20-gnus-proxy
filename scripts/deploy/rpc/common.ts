...
export function createRPCConfig(
  ...
  if (options.useHardhatConfig !== false && !options.rpcUrl) {
    if (options.verbose) {
      console.log(
        chalk.blue(
          "📋 Using hardhat configuration for diamond and network settings",
        ),
      );
    }
    ...
    try {
      ...
      console.error(
        chalk.red(
          `❌ Failed to create config from hardhat: ${(error as Error).message}`,
        ),
      );
      console.error(
        chalk.yellow(
...
export function createMainCommand<T extends BaseRPCOptions>(
  ...
  return optionBuilder(
    addCommonOptions(
      program.command(`${commandName} ${args}`).description(description),
    ),
  ).action(
    async (diamondName: string, networkName: string, options: Partial<T>) => {
      try {
