...
async function validateABIs(
  ...
  for (const [facetName, facetData] of Object.entries(facets) as [
    ...
    try {
      ...
      console.log(
        chalk.green(
          `   ✅ ${facetName}: Contract exists (${code.length} bytes)`,
        ),
      );
      validCount++;
    } catch (error) {
      console.log(
        chalk.red(
          `   ❌ ${facetName}: Validation failed - ${(error as Error).message}`,
        ),
      );
    }
  }
...
async function validateSelectors(
  ...
  try {
    ...
    console.log(
      chalk.blue(`📦 Deployed facets: ${Object.keys(deployedFacets).length}`),
    );
    ...
    for (const onChainFacet of onChainFacets) {
      ...
      const matchingFacet = Object.entries(deployedFacets).find(
        ([, facetData]: [string, any]) =>
          facetData.address.toLowerCase() ===
          onChainFacet.facetAddress.toLowerCase(),
      );
      ...
      if (matchingFacet) {
        ...
        if (
          deployedSelectors.length === onChainFacet.functionSelectors.length
        ) {
          console.log(
            chalk.green(
          ...
          console.log(
            chalk.yellow(`   ⚠️  ${facetName}: Selector count mismatch`),
          );
          console.log(
            chalk.yellow(
        ...
        console.log(
          chalk.red(`   ❌ Unknown facet: ${onChainFacet.facetAddress}`),
        );
      }
    }
    ...
    console.error(
      chalk.red(`❌ Selector validation failed: ${(error as Error).message}`),
    );
  }
}
...
async function compareOnChainState(
  ...
  console.log(
    chalk.green(`✅ Diamond contract verified (${diamondCode.length} bytes)`),
  );
  ...
  try {
    ...
    const proxyDiamond = new ethers.Contract(
      diamondAddress,
      gnusDaoABI,
      provider,
    );
    const protocolVersion = await proxyDiamond.protocolVersion();
...
async function verifyDeployment(options: VerifyOptions): Promise<void> {
  ...
  await showPreOperationInfo(config, "Diamond Verification", {
    ...
    "🔑 Etherscan API": options.etherscanApiKey
      ? "Configured"
      : "Not configured",
  ...
  console.log(
    chalk.blue(
      `🔍 Starting verification of diamond "${config.diamondName}"...`,
    ),
  );
